...
class V1AdInmemoryApiTest {
    ...
    fun read() = testApplication {
        ...
        val responseObjCreate = responseСreate.body<AdCreateResponse>()

        val response = client.post("/v1/ad/read") {
            val requestObj = AdReadRequest(
                requestId = "12345",
                ad = AdReadObject(responseObjCreate.ad?.id),
                debug = AdDebug(
                    mode = AdRequestDebugMode.TEST,
                )
            )
        ...
        assertEquals(200, response.status.value)
        assertEquals(responseObjCreate.ad?.id, responseObj.ad?.id)
    }
