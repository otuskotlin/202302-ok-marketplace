...
fun FunSpec.testApiV1(client: Client, prefix: String = "") {
    context("${prefix}v1 prod") {
        ...
        test("Update Ad ok") {
            val created = client.createAd(mode = prod)
            client.updateAd(
                created.id, created.lock,
                AdUpdateObject(
                    title = "Selling Nut",
                    description = someCreateAd.description
                ), mode = prod
            )
        ...
        test("Search Ad ok") {
            val created1 = client.createAd(someCreateAd.copy(title = "Selling Bolt"), mode = prod)
            val created2 = client.createAd(someCreateAd.copy(title = "Selling Nut"), mode = prod)
        ...
        test("Offer Ad ok") {
            ...
            withClue("Find offer for supply") {
                val results = client.offersAd(supply.id, mode = prod)
                results shouldHaveSize 1
                results shouldExist { it.id == demand.id }
            }

            withClue("Find offer for demand") {
                val results = client.offersAd(demand.id, mode = prod)
                results shouldHaveSize 1
                results shouldExist { it.id == supply.id }
            }
        }
    }
}